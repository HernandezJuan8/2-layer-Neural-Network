...
Juan Hernandez G01234626 jherna56 section 1

REPORT ALL THE WAY AT THE BOTTOM!!!!!

Write a two-layer backpropagation neural network.

Due the evening of MONDAY, March 21, at MIDNIGHT.
 :(

You will send this project, filled out and working, as a SINGLE FILE
to your TA as an attachment
...
one matrix is multiplied against the *corresponding*
element in the other matrix (the two matrices must
of course be the same shape). So for each element
Aij and Bij in the matrices A and B, the value of
the element Cij in matrix C would be Aij * Bij.
Let's say that two vectors are [1 2 3] and [5 6 7].
Then the result would be [1*5 2*6 3*7] = [5 12 21].
sigmoid[] sigmoid function applied to each element
tr[] transpose the matrix
1 - subtract all elements in the matrix from 1
- subtract two matrices
+ add two marices
alpha (...) multiply all elements in the matrix by the scalar
value alpha
...
a binary classification problem. Maybe you recognize this problem.
...
EXTRA CREDIT. If you like you might try one or two of the following. Add a sentence in the body of your email to alert the TA that you are attempting extra credit. Please do not submit extra credit unless you're pretty sure it's correct: half-baked attempts get no credit and it just puts a lot of burden on the TAs.
...
(float (/ 1 (+ 1 (exp (* -1 u)))))) ;;Returns float value of sigmoid (float to not make it a fraction)
...
(* .5 (first (first (multiply (transpose (subtract correct-output output)) (subtract correct-output output))))))
;; THIS NEEDS TESTING TO MAKE SURE ITS A 1X1M to continue
...
the datum as input."
(let ((h (make-matrix 1 (length (first datum))
		      (map-m (lambda (i) (sigmoid i))
			     (multiply v (first datum))))))
  (make-matrix 1 (length (second datum))
	       (map-m (lambda (i) (sigmoid i))
		      (multiply w h)))))
...
;; same let* statement.
(let* ((o (forward-propagate datum v w)) ;; output vector using forward propagate
       (h (make-matrix 1 (length (first datum))
		       (map-m (lambda (i) (sigmoid i))
			      (multiply v (first datum))))) ;; h vector using piece from forward propagate
       (odelta (e-multiply (subtract (second datum) o) o (subtract-from-scalar 1 o)))
       ;; odelta calculate from using expected output, and output vector 
       (hdelta (e-multiply h (subtract-from-scalar 1 h) (multiply (transpose w) odelta))))
  ;;h delta calculated from using h vector and using odelta
  ;;BELOW is new list containing (v w)
  (setf w (add w (scalar-multiply alpha (multiply odelta (transpose h))))) ;; NEW W modifed 
  (setf v (add v (scalar-multiply alpha (multiply hdelta (transpose (first datum)))))) ;; new V modified
  (list v w))) ;; list in order of (v w) respectively
...
and the final W matrix of the learned network."
  (let ((v (make-random-matrix num-hidden-units (first (first data)) initial-bounds))
	(w (make-random-matrix (second (first data)) num-hidden-units initial-bounds))
	(i 0)
	(shuffled nil)
	(temp nil))
    (loop while (< i max-iterations) do
      (setf shuffled (shuffle data)) ;; shuffle data
      (dotimes (j (length shuffled))
	(setf temp (back-propagate (elt shuffled j) alpha v w))
	(setf v (first temp))
	(setf w (second temp))) ;; backpropagate the data
      (incf i)
      (if (= (mod i modulo) 0)
	  (let ((allErrors '())
		(d nil)
		(e nil))
	    (dotimes (j (length shuffled))
	      (setf d (forward-propagate (elt shuffled j) v w))
	      (setf e (net-error d (second (elt shuffled j))))
	      (if print-all-errors

		  ;;print all errors here lol
		  ;; use net error? lmao
		  (print e)
		)
	      ;;compute stats for worst err n mean err
	      (nconc allErrors e)
	      )
	    (print "Mean error:")
	    (print (average allErrors))
	    (print "Worst Error:")
	    (print (first (sort (copy-list allErrors) '<)))
	    ;;Prints worst error and mean error
	    ;; check if worst err is les than min err
	    ;; if so then quit
	    (if (> (first (sort (copy-list allErrors) '<)) *a-good-minimum-error*)
		(return-from net-build (list v w)))))) ;; exit net-buld since not much change
    (list v w)))
...
;;;; This data isn't normalized. I suggest scaling the data to between 0.1 and 0.9
...
Implementation of sigmoid function follows exactly as the notes do of doing the function
the function is 1/(1+e^-u) it is multiplied by -1 for bias wth u. It returns as a float just
in case it wanted to return a fraction. For my net-error function I followed the function 
also given and using matrix manipulation to get it as a 1x1 matrix then extracting it and
multiplying by half to give me the net-error. For the forward propagation function. I only
used the the input and used the sigmoid function to get every vector from input and matrix v
to turn into a column vector then using that I used the sigmoid function with matrix w and 
the new column vector i made and returned that as the output column vector.
For the back propagate function I obtained odelta using the the forward propagate function
and some matrix manipulation. hdelta used h which a piece of forward propagate was used to
get h and more matrix manipulation. Once i had hdelta and o delta I can for first used odelta
and add it with the w matrix and the hdelta as well and do same operations as i did on w but
on v and instead used input to get the back propagation.
For net build I star by creating some w and v matrices using the make matrix function
but with random so it can init the matrix with both -initbound and initbounds. 
then create a loop for to go through max iterations. In the loop I do shuffle data to shuffle
the data around and set it to a a variabled called shuffled. Then I go through each
datum and do the back propagate the function using the shuffled data and w and v .
it then increases i. Then it checks if i%modulo == 0 to then start seeing the
errors. I created more variables , all errors starting as an empty list.
d and e to hold temp values. then the next bit will go through each datum and forward
propagate each datum to get an output vector. using that it will then calculate
the neterror using the output it recieved and the output in the datum.
if printing all errors is one then it will print out all the errors of the NN.
it will then use the nconc to destructively append the error of the current
datum to have a list of all the errors. It will then print the mean error 
by adding up all the errors and dividing by the length of the allErros list.
It will then print out the worst value by first sorting the list in
ascending order and grabbing the first item in allErrors.
lastly it will check worst error is lower than the min error and if so
then it will return prematurely.
otherwise once the function if fully done running it's course it will return a list
of (v w) repsenting the matrix. 
I could not implement generalization.
I learned that most operation done can be done easily with matrix manipulation than some
complicated math which I was doing at first. I am not confident that this will provide
correct answers. 
